...
module.exports = (app) => {
  ...
  app.route('/login')
    .get(await_error_handler(async (req, res, next) => {
      return await res.sendFile(path.join(__dirname, '../login.html'));
    }))
    .post(passport.authenticate("local"), await_error_handler(async (req, res, next) => {
  ...
  app.get("/api/user_data", is_authenticated, async (req, res) => {
